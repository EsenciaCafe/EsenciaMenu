...
function tags(item){if(!SHOW_ALLERGENS)return '';const a=item.allergens;return `<div class="allergen-tags">${a.contains.map(x=>`<span class="tag ${isSelected(x)?'warning':''}">${esc(allergenLabel(x))}</span>`).join('')}${a.traces.map(x=>`<span class="tag ${isSelected(x)?'warning':''}">${t('Trazas: ','Traces: ')}${esc(allergenLabel(x))}</span>`).join('')}</div>`;}
