...
// Enable only after the allergen information has been confirmed.
const SHOW_ALLERGENS = false;
...
function tags(item){if(!SHOW_ALLERGENS)return '';const a=item.allergens;return `<div class="allergen-tags">${a.contains.map(x=>`<span class="tag ${isSelected(x)?'warning':''}">${esc(allergenLabel(x))}</span>`).join('')}${a.traces.some(isSelected)?`<span class="tag warning">${t('Posibles trazas','Possible traces')}</span>`:''}<span class="tag pending">${t('En revisión','Under review')}</span></div>`;}
...
function allergenBox(entries){
  ...
  const pending=entries.filter(p=>p.i.allergens.pending);
  return `<div class="allergen-box"><div class="draft-label">${t('BORRADOR · PENDIENTE DE VALIDACIÓN','DRAFT · AWAITING VALIDATION')}</div><h3>${t('Información de alérgenos','Allergen information')}</h3><p><strong>${t('Contiene — identificado:','Contains — identified:')}</strong></p>${a.contains.length?`<div class="allergen-tags">${a.contains.map(x=>`<span class="tag ${isSelected(x)?'warning':''}">${esc(allergenLabel(x))}</span>`).join('')}</div>`:`<p>${t('No se han identificado alérgenos en los datos disponibles. Esto no acredita su ausencia.','No allergens identified in the available data. This does not establish their absence.')}</p>`}<p><strong>${t('Posibles trazas:','Possible traces:')}</strong> ${a.traces.length?a.traces.map(x=>esc(allergenLabel(x))).join(', '):t('Sin información verificada sobre trazas.','No verified trace information.')}</p><p class="review-note">${t('Información provisional. Confirma tu elección con nuestro equipo antes de pedir. Las sustituciones y la preparación pueden cambiar los alérgenos.','Provisional information. Check with our team before ordering. Substitutions and preparation can change allergens.')}</p>${pending.length?`<details><summary>${t('Qué falta confirmar','What still needs checking')}${en()?' · ES':''}</summary><ul class="recipe-notes">${pending.map(p=>`<li><strong>${esc(field(p.i,'name'))}:</strong> <span lang="es">${esc(p.i.allergens.pending)}</span></li>`).join('')}</ul></details>`:''}<details><summary>${t('Origen de la información','Information source')}${en()?' · ES':''}</summary><p>${t('Cruce interno de carta y alérgenos · 23/09/2026. No es una declaración final aprobada.','Internal menu and allergen crosswalk · 23 September 2026. This is not a final approved declaration.')}</p>${entries.map(p=>`<p lang="es"><strong>${esc(p.i.name)}:</strong> ${esc(p.i.allergens.basis||'Pendiente de documentar.')}</p>`).join('')}</details></div>`;
}
function openProduct(k){
  ...
  $('#product-content').innerHTML=`<div class="dialog-art ${image?'has-photo':''}" style="--tone:${g.tone}">${art(g.art)}${photo(image,field(p.i,'name'),'detail-photo',p.i.image_frame,'detail')}</div><div class="dialog-body"><p class="eyebrow">${esc(field(p.s,'title'))}</p><h2 id="product-title">${esc(field(p.i,'name'))}</h2>${field(p.i,'desc')?`<p class="dialog-description">${esc(field(p.i,'desc'))}</p>`:''}${field(p.s,'subtitle')?`<p class="reduced-info">${esc(field(p.s,'subtitle'))}</p>`:''}<div class="dialog-price">${esc(price(p.i))}<small>${t('IGIC incluido','IGIC tax included')}</small></div><div>${tags(p.i)}</div><button class="text-link" data-show-allergens hidden>${t('Ver información de alérgenos ↓','View allergen information ↓')}</button>${field(p.s,'note')?`<p class="section-note">${esc(field(p.s,'note'))}</p>`:''}${extrasCatalogue(extras,p)}<div id="product-allergens">${allergenBox([p])}</div><p class="reduced-info">${t('Si tienes alguna alergia, consulta los ingredientes y la preparación con nuestro equipo antes de pedir.','If you have any allergies, ask our team about ingredients and preparation before ordering.')}</p></div>`;
